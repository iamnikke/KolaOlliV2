...
const myGlobe = Globe()(document.getElementById('globeViz')).
    ...
    .onLabelHover(label => {
      document.getElementById('globeViz').style.cursor = label
          ? 'pointer'
          : 'default';
    })
...
const directionalLight = myGlobe.lights().
...
document.getElementById('btn-fly').addEventListener('click', async () => {
  ...
  const colaLoad = parseInt(document.getElementById('ui-cola-quantity').value);
  const selectedPlane = document.querySelector('.plane-card.selected');
  const planeCapacity = parseInt(selectedPlane.getAttribute('data-capacity'));
  ...
  try {
    ...
    if (data.success) {
      ...
      if (data.caught) {
        ...
        document.getElementById('bribe-fine-text').textContent =
            `Jäit kiinni ylilastista! Sakko: ${data.fines}€`;

        // Show ONLY Bribe Screen
        document.getElementById('ui-success-screen').style.display = 'none';
...
async function submitBribeChoice(choice) {
  try {
    ...
    if (data.success) {
            ...
            // Hide bribe screen and NOW show the success screen
            document.getElementById('ui-bribe-screen').style.display = 'none';
            alert(data.message);
            showSuccessScreen();
...
function showSuccessScreen() {
    ...
    // Update the profit message based on what happened in the move_player call
    const profitMsg = document.getElementById('success-sales-message');
    if (currentSalesProfit > 0) {
        profitMsg.textContent = `Myit colat ja tienasit ${currentSalesProfit.toFixed(2)} €! (Rahat lisätään kun palaat kotiin)`;
    } else {
        profitMsg.textContent = `Et ottanut colaa mukaan, joten et tienannut mitään.`;
    }
      ...
      try {
        ...
        if (data.success) {
          // päivitä ui
            ...
            currentSalesProfit = 0; // Reset for next trip
